...
% 可引用：canonical replay-ready panel + baseline audit artifacts (tab_baseline_consistency / fig_fliprate_by_season).
...
\paragraph{Conservative imputation for counterfactual survivors.}
In counterfactual replays, a rule change may keep a contestant active after the week where they exited in the observed data; in that case, the contestant's subsequent-week inputs are unobserved. To complete the replay without injecting trend assumptions, we use a minimal-information imputation: (i) the next-week judges' total score is filled by the contestant's mean-to-date,
\[
\hat{J}_{i,t+1}=\frac{1}{t}\sum_{k=1}^{t}J_{i,k},
\]
and (ii) the next-week fan vote share is carried forward from the most recent inferred value,
\[
\hat{p}_{i,t+1}=\hat{p}_{i,t}.
\]
This imputation is used only to extend counterfactual trajectories and is not intended to forecast future performance; consequently, any rule-comparison statistics that depend on these imputed segments should be interpreted as conservative estimates of rule impact.
...
We compute KPI1/KPI3 over counted (season, week) instances under a unified replay convention.
KPI1 (Jaccard set agreement) summarizes week-level agreement between the observed eliminated set and the
replayed eliminated set, including no-elimination weeks via an explicit empty-set convention.
KPI3 (FlipRate) measures how often the predicted eliminated set differs between percent-based and rank-based
replays under the same inputs.
