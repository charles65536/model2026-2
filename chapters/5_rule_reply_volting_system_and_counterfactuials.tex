...
High FlipRate indicates a season or era where the two aggregation paradigms can meaningfully change who is eliminated, which is a direct proxy
for controversy risk when rules change. These metrics motivate the targeted case studies below, which explain \emph{why} divergences occur.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{output/fig/bias\_by\_season\_alpha0p5.png}
    \caption{Average rank difference (bias) of eliminees vs. survivors across seasons by methods (percent/rank/Rule A~\ref{subsubsec:d2_ruleA}/Rule B~ \ref{subsubsec:d2_ruleB}) at $\alpha=0.5$.}
    \label{fig:bias_by_season_alpha0p5}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{output/fig/bias\_mean\_by\_method\_alpha0p5.png}
    \caption{Method-level average summary of bias}
    \label{fig:bias_mean_by_method_alpha0p5}
\end{figure}
