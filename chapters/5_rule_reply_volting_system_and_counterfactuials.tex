...
Following the official rule definitions, we conduct season--week counterfactual replays under
(i) rank-based aggregation and (ii) percent-based aggregation.
Each replay starts from the observed eligibility labels and updates the \texttt{active} set
week-by-week according to the counterfactual eliminations under the chosen rule, producing a
replay path that can be directly compared across rules and seasons.
All panel construction conventions and any conservative completion rules are defined once in
Section~3.1 and are not repeated here.
This chapter summarizes cross-season differences (Section~\ref{subsec:rank_vs_percent_across_seasons}), explains divergences via controversial case studies (Section~\ref{subsubsec:replay_protocol_and_eligibility}),
evaluates the incremental impact of bottom-two and judges' save \ref{subsubsec:cross_season_summary_metrics}, and synthesizes implications (Section~\ref{subsec:controversial_contestants_case_studies}).

\subsection{Rank vs. Percent Across Seasons}\label{subsec:rank_vs_percent_across_seasons}
We first report a reproducible sanity baseline (BL-0): judge-only information with a uniform fan proxy; we then run full replays using Task~A inferred votes.
