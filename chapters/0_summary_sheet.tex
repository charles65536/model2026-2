...
\begin{itemize}
    ...
    \item Mixed-effects attribution models (Task D1) separating drivers of judges' scores and fan support, and a professionalism-first institutional redesign (Task D2):
    a judges-only gating risk set followed by a fan-aware decision within the gate, with late-stage structured suspense mechanisms (bottom-$k$ band / limited multi-elimination)
    evaluated through replay and uncertainty stress tests.
\end{itemize}
...
\begin{itemize}
    ...
    selected via replay-based robustness tests under vote uncertainty.
\end{itemize}

\subsection*{Management recommendation}
Pilot the proposed judges-gated two-stage rule in a one-season ``shadow mode'' (computed in parallel with the broadcast rule) and adopt a parameter region that preserves replay fidelity
on eligible weeks while improving professionalism and robustness under vote-uncertainty perturbations.
