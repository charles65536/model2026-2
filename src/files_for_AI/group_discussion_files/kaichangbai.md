...
* 文件说明 / 权威口径：
- 2026_MCM_Problem_C.pdf：题面信息（权威）
- 2026_MCM_Problem_C_Data.csv：原始数据（权威，最重要）
- data_play_book.md：你的 playbook（需要遵循）
- visual_play_book.md：下游 visualization_agent 的 playbook（了解即可，次要）
- dynamic_files/context_pack_for_ai.md 与 dynamic_files/kpi_registry.md：本项目“唯一权威口径”（如有冲突以它们为准）
- dynamic_files/data_agent_triage_ticket.md：给你的 Triage 工单（必须逐条满足）

* 禁止：
- 不要读取/引用 problem_C_investigation.md 或任何未被上面列为“权威/必需”的文件内容来定义字段、KPI、单位或题意；不确定则标注“待确认”，采用工单里给的最安全默认。

* 你的任务（严格按工单 DoD）：
按 dynamic_files/data_agent_triage_ticket.md 实现：
P0 清洗 → KPI 可算 → baseline 可跑 → 产出
1) output/table/tab_baseline_consistency.tex
2) output/table/intermediate_weekly_panel.csv（或工单要求的中间 csv）
并在回复中明确列出你生成的文件路径与它们对应的 KPI/切片。

* 工程约束：
- 代码入口脚本放在 src/ 下，并确保可直接运行生成上述产物（manifest 指向入口）。
- 不做泛泛 EDA；只做为 KPI 与 baseline 对照服务的最小清洗/切片/汇总。

* 验收以文件产物为准：如果没有生成 output/table/tab_baseline_consistency.tex以及中间.csv文件，则视为未完成。
